...
reduceOver reducer neutral iterator from to body =
    if from > to then
        ...
        let
            evaluatedIteration =
                betaReduceStep (from |> toString |> Id) (iterator) body |> (\x -> numMath x [])
        in
...
reduceOverResult reducer neutral interator from to body =
    case ( from, to ) of
        ( Ok fromOk, Ok toOk ) ->
            reduceOver reducer neutral interator fromOk toOk body
        ...
        case exp of
                ...
                let
                    mapperResult =
                        case opType of
                            ...
                            _ ->
                                Err ( (ExpBelow exp |> brc), "invalid operator" )
                in
                    case mapperResult of
                        Ok mapper ->
                            Result.map2 mapper (numMath left (BinOpLeft opType opInfo right |> brc)) (numMath right (BinOpRight opType opInfo left |> brc))
            ...
            UnaryOp opType opInfo inner ->
                ...
                in
                    case mapperResult of
                        Ok mapper ->
                            Result.map mapper (numMath inner (UnaryOpHere opType opInfo inner |> brc))
            ...
            BigOp opType opInfo to (BinOp Equals eqInfo iterator from) over ->
                let
                    breadCrumFrom =
                        breadCrum ++ [ (BigOpUnder opType opInfo to over), (BinOpRight Equals eqInfo iterator) ]

                    breadCrumTo =
                        breadCrum ++ [ (BigOpOver opType opInfo from over) ]

                    mapperResult =
                        case opType of
                            BigSum ->
                                (\fromVal toVal -> reduceOverResult (+) 0 iterator fromVal toVal over) (numMath from breadCrumFrom) (numMath to breadCrumTo)

                            BigProd ->
                                (\fromVal toVal -> reduceOverResult (*) 1 iterator fromVal toVal over) (numMath from breadCrumFrom) (numMath to breadCrumTo)
            ...
            s ->
                Result.Err ( breadCrum, (Debug.log "not impl" s) |> toString )
